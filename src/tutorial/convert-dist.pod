...
(If you're using Pod::Weaver, make sure that this line isn't being lost in
translation!  One way to do that is to make sure you've got a Leftovers
section.)
...
@Basic gets us a few other plugins, too, but most of them aren't worth
...
Prereqs and add AutoPrereqs.  (In this example, it's fine, but AutoPrereqs may
not pick up some unusual forms of dependency.  When converting your own dist,
consider double-checking.)
